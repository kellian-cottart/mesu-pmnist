...
def presynaptic(lr=0.01,
    ...
    def update(gradients, state, params):
        ...
        def update_presynaptic(param, grad):
            ...
            probability_update = where(grad.probability > abs(g_lim), p_up, - p_down)
            frozen_mask = where(param.probability > p_freeze, 0.0, 1.0)
            frozen_mask = where(param.probability < p_max - p_freeze, 0.0, frozen_mask)            
            ...
            strength_update = (param.probability / probability_next - 1) * param.strength - learning_rate / (param.probability * probability_next) * grad.strength
