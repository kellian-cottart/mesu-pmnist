...
def foovbmatrixvariate(
    ...
    def update(gradients, state, params=None):
        ...
        def update_matrixvariate(param, grad):
            ...
            mu = param.mu - lr_mu * ((param.sigma_2 @ param.sigma_2.T) @ grad.mu) @ (param.sigma_1 @ param.sigma_1.T) 
            sigma_1 = solve_matrix_equation(param.sigma_1, grad.sigma_1)
            sigma_2 = solve_matrix_equation(param.sigma_2, grad.sigma_2)
