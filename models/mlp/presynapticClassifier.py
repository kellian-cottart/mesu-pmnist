...
class BasePresynapticMLP(Module):
    layers: list[PresynapticLinear]
    ...
    def __init__(self, key, layers=[784, 50, 10], use_bias=False, activation_fn=None, norm_fn=None, norm_params=None, **kwargs):
        ...
        # Start with `ravel`
        self.layers = []
        # Add `PresynapticLinear` and `relu` alternately except after the last linear layer
        for i in range(len(layers) - 1):
            # Add `PresynapticLinear` layer
            self.layers.append(PresynapticLinear(
    ...
    def __call__(self, x, state, key, *, backwards=False):
        ...
        x = ravel(x)
        x = forward(x, self.layers, key=s_key)
        return x, state


def forward(x, layers, key):
    for i, layer in enumerate(layers):
        if isinstance(layer, PresynapticLinear):
...
class BasePresynapticMLPLayerNorm(BasePresynapticMLP):
